unit lazExt_aBTF_CodeExplorer_DEBUG;
...
type
  ...
  TlazExt_aBTF_CodeExplorer_wndDBG = class(TForm)
    a_StayOnTop: TAction;
    a_Clear: TAction;
    a_Save: TAction;
...
var _WndDBG_:TlazExt_aBTF_CodeExplorer_wndDBG;

procedure DEBUG_window_SHOW;
begin
    if not Assigned(_WndDBG_) then _WndDBG_:=TlazExt_aBTF_CodeExplorer_wndDBG.Create(Application);
...
procedure TlazExt_aBTF_CodeExplorer_wndDBG.FormClose(Sender:TObject; var CloseAction:TCloseAction);
begin
    CloseAction:=caFree;
   _WndDBG_:=NIL;
end;

procedure TlazExt_aBTF_CodeExplorer_wndDBG.FormCreate(Sender: TObject);
begin
...
procedure TlazExt_aBTF_CodeExplorer_wndDBG.a_ClearExecute(Sender: TObject);
begin
    memo1.Clear;
end;
...
procedure TlazExt_aBTF_CodeExplorer_wndDBG.a_StayOnTopExecute(Sender: TObject);
begin
    if self.FormStyle=fsStayOnTop then self.FormStyle:=fsNormal
    else self.FormStyle:=fsStayOnTop;
end;

procedure TlazExt_aBTF_CodeExplorer_wndDBG.a_StayOnTopUpdate(Sender: TObject);
begin // ????
...
procedure TlazExt_aBTF_CodeExplorer_wndDBG.Message(const TextMSG:string);
var tmp:string;
begin
...
procedure TlazExt_aBTF_CodeExplorer_wndDBG.Message(const msgTYPE,msgTEXT:string);
begin
    if msgTYPE<>''
